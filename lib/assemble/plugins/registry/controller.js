...
module.exports = {
    ...
    createRegistry: function(registry) {
        return new Promise(function(resolve) {
            ...
            writeFile((registry || {}).file || upath.join(process.env.MODULE_DEV ? 'test' : 'src', 'js', 'packages.js'), template({
                sources: list
            }), function() {
...
function addControllersToList(nodes) {
    ...
    for (var i = 0; i < nodes.length; i++) {
        ...
        controller = controller.replace(RegExp(process.env.npm_package_name + '\/.*'), upath.normalizeSafe(upath.join('..', '..', 'src')));
        list.push({
            name: name,
