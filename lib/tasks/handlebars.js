...
module.exports = function(name, config, watch) {
    ...
    app.layouts.option('base', config.options.layouts.options.base);
    app.partials.option('base', config.options.partials.options.base);

    gulp.task('handlebars_update', function() {

        app.layouts(config.options.layouts.files.src);
        app.partials(config.options.partials.files.src);
    ...
    return taskGenerator(name, config, watch, function(taskName, task) {
        ...
        runSequence.call(null, ['handlebars_update'].concat(tasks), function() {
            ...
            registry.createRegistry().then(function() {
                cb();
            });
        });
    });
